...
from alma_pixel_fitting import CreateEmptyModel, read_fits_file, get_velocity
...
def create2Dimg(imgsize, modelList, comp, key, sortedPrefixes):
    ...
    # Create base image
    img = {'value': np.empty( (imgsize,imgsize) ),
           'error': np.empty( (imgsize,imgsize) )}
...
def produceFitsOutput(img, almaimg, fitsname):
    ...
    def copyALMAkeywords(hdr, almaimg):
        ...
        tocopy = ['bmaj', 'bmin', 'bpa', 'object', 'radesys', 
...
def findClosestComponent(ind, filledAdjacent, prefixList, modelList):
    ...
    allDistances = [ distanceBetweenComponents(ind, adj, prefixList, modelList) 
...
def distance(model1, model2, p1, p2):
    ...
    m = 2
    n = 1
    dist = (vdist**m * adist**n)**(1./(m+n))
    return dist

def sortPrefixes(modelList, algorithm='iterative', **kwargs):
    ...
    for N in range(1,maxComps+1):   # N is the component number of the Gaussian
        ...
        while len(trimmedModelList) != 0:
            ...
            for ind in modelList:
                ...
                filledAdjacent = [ adj for adj in adjacent
                ...
                closestPrefix = findClosestComponent(ind, filledAdjacent, prefixList, modelList)
...
def velocity_matching(modelList, velocities=[-200,200], **kwargs):
    ...
    for ind in modelList:
        ...
        while len(unusedPref) != 0:
            ...
            index_pairs = np.array(np.meshgrid(range(len(unusedPref)), range(len(remainingComps)) )).T.reshape((-1,2))
...
def makePrefixList(modelList):
    ...
    prefixList = {}
    for ind in modelList:
        prefixList[ind] = { i:comp.prefix for (i,comp) in enumerate(modelList[ind].components[1:]) }
    return prefixList
